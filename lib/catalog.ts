...
export async function searchCatalog({
  ...
  // table. `viewFailed` drops the teacher clause if migration 0027 hasn't run
  ...
  // The teachers column arrives with 0027; before it, run the query without
  // that clause rather than failing the whole search.
  if (error && teachers !== "all" && /teachers|column/i.test(error.message ?? "")) {
    viewFailed = true;
    // "only" has nothing to list pre-0027 — no book can be flagged yet.
