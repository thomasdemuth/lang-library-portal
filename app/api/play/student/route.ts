 ...
 * public_id. Shares only what's meant to be shared: display name, profile
 * photo, books-read count, favorites, and collections — never the email.
 * Hidden profiles 404 before any of it (photo included) is read.
 */
export const GET = guarded(async (req: NextRequest) => {
  ...
  // photo_url arrives with migration 0020, hidden with 0013 — peel columns
  // off the select until it matches the live schema.
  let { data: profile, error } = await db()
    .from("student_profiles")
    .select("email, hidden, photo_url")
    .eq("public_id", id)
    .maybeSingle();
